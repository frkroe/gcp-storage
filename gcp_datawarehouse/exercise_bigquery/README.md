...
In this exercise, we will use the `public datasets` of BigQuery to test the different capabilities of the tool.
...
We will focus on the [ethereum dataset](https://console.cloud.google.com/marketplace/product/bigquery-public-data/blockchain-analytics-ethereum-mainnet-us). This dataset contains the historical data of the Ethereum blockchain. We will make a few queries to test the capabilities of BigQuery.
...
However, this query can be further optimized by filtering the data of the tables before joining them. This can be done by encapsulating both tables with CTEs (Common Table Expression) and then joining them.
...
If we now check both execution details, we will see that the second query uses fewer slots than the first one.
