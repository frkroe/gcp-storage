...
bigtable_client = bigtable.Client.from_service_account_json(
    os.getenv("GOOGLE_APPLICATION_CREDENTIALS_BIGTABLE"), project=BIGTABLE_PROJECT_ID
)
...
def callback(message):
    try:
        ...
        row.set_cell(
            "measures", "sent_at", sent_at.encode("utf-8")
        ...
        row.commit()
        print(
            f"Inserted row {row_key.decode('utf-8')} into Bigtable with sent_at: {sent_at}"
        )
...
def main():
    ...
    print(
        f"Listening for messages on {SUBSCRIPTION_ID} from "
        f"{PUBSUB_PROJECT_ID} and writing to Bigtable in "
        f"{BIGTABLE_PROJECT_ID}..."
    )
