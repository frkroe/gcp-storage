...
```sh
cd gcp-storage/gcp_datalake/exercise_end2end/terraform
```
...
Once we finish the exercise, we can destroy the infrastructure by running:
...
2. Clone or pull latest changes from this repository.
...
Now we can deploy the EL pipeline to synchronize the PostgresDB database with BigQuery.
...
3. Run the script to synchronize the PostgresDB database
   ```sh
   POSTGRES_IP=<postgres-ip> GCP_PROJECT=<your-project-id> python -m analytical_layer.el_orders.main
   ```
...
Now, you have a template prepared with the modifications needed to create the views for the delivery events.

1. Go to the folder `dbt_template` in the `exercise_end2end` directory.
...
1. Go to the exercise directory in your local machine:
   ```sh
   cd gcp-storage/gcp_datalake/exercise_end2end
   ```
